...
- 个人版已移除原项目 Google AdSense 脚本和广告组件，减少第三方脚本对加载和离线体验的影响。
...
当前缓存版本为 `rocom-pwa-v5-*`。升级后会清理旧版本缓存，避免继续使用旧 app-shell、旧数据 URL、旧图标或已被纠正的技能图片。
...
1. DevTools > Application > Cache Storage 是否出现 `rocom-pwa-v5-app-shell`。
2. `rocom-pwa-v5-app-shell` 中是否包含 `/index.html` 或站点完整 URL 对应的 `index.html`。
...
6. 确认 Cache Storage 中出现 `rocom-pwa-v5-app-shell`，并且其中有 `/index.html`。
