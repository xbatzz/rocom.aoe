...
`form` 必须优先使用 `PETBASE_CONF.form` 的官方描述并原样写入生成数据。只有原始字段为空时，才允许从带括号的展示名称、图鉴名称差异或头像资源后缀推断；不要用资源名推断结果覆盖官方描述。`yarn test:pet-data-quality` 会逐条校验所有非空原始 `form`，防止以后同步新精灵时再次丢失具体形态名。
