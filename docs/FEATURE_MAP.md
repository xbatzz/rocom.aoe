...
项目当前已有移动端优先的 `/pvp-lite` 对战助手和参数更完整的 `/pvp` 详细版。`/pvp-lite` 保持结果优先，默认读取当前激活队伍，一屏内突出双方选择、速度结论、属性对位、推荐技能纸面伤害、双方本系 / 非本系一击威力线和联防候选；详细公式、倍率和不可计算技能说明放在折叠区，适合作为手机实战前快速查看入口。一击威力线按双方当前临时构筑的主攻项计算，并提供双方 0%-100% 当前生命滑块同步更新结果；本系取自身属性中对目标倍率更高的一项，非本系按等倍覆盖属性估算，仅作为固定威力单段技能的理论参考。`/pvp` 保留原详细版页面，用于查看完整参数面板和更细的计算说明。
