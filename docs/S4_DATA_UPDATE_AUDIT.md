...
本次更新的维护经验见 [S4 更新问题与经验总结](./S4_UPDATE_LESSONS.md)。其中第 7 节明确区分实装证据与资源完整性；下文记录的头像可用性条件是上一轮实现，尚未落实该独立判断原则。
...
**后续确认：云梦豚 3784（图鉴 467）和长江豚 3785（图鉴 468）为当前客户端预载记录，不能仅凭 PETBASE 和图鉴配置认定为完整实装。**

用户已确认在 FModel Loading Mode = All 下，对应 Pet1024/Pet256 package 仍不存在；这是当前客户端尚未完整投放资源，不是 FModel 导出失败。PetOutline1024 的同名资源不属于 PETBASE 引用目标，不能替代。PETBASE 已引用以下普通/异色头像及 Pet256 下同名资源：
...
此前徽章测试因此失败。后续最小修改在同步脚本中保留原有图鉴、战斗内容及首领证据判断，并额外要求主头像引用对应真实 Pet1024 导出或已有导入头像；只存在配置而缺少资源的记录保留数据，但不标记为完整实装。没有 ID 白名单，不使用 PetOutline1024。此资源完整性条件用于项目展示，不把本地文件缺失普遍断言为客户端未投放；上述两条客户端 package 缺失结论来自用户确认。其余 20 个缺头像引用为原有未实装记录；4 个缺技能图标键为原有 `700025`、`btn_huangchong_png`、`img_BossSkill`、`img_BossSkill1`。特性图标没有缺失。
...
无未确定的嵌套二进制布局，没有未解析引用或伪 ID 遗留；3784/3785 已按用户确认记录为客户端预载、资源尚未完整投放。保留工作区变更，未提交或 push，也未修改/引用 NRC_S3_backup 的内容。
