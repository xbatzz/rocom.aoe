...
已确认游戏解包文件 `Species.png` 就是队伍导出图使用的属性图标图集。原图尺寸为 `512×256`，有效图标位于左上角约 `282×232` 区域，按 5 列 × 4 行排列；其余区域透明。共享原图现位于 `src/assets/Species.png`，坐标映射位于 `src/lib/typeIcons.ts`，图片导入与站内属性标签复用同一资源和映射，不再在导入功能内部维护副本。
...
图集单元格按 `56×58` 的步长裁剪。站内 UI 通过 SVG 视口引用共享图集并收紧外沿像素，图片导入识别仍使用完整单元格，避免视觉裁切影响模板匹配。后续调整坐标时必须同时验证页面显示和全部图标的导入匹配结果。
