...
S4 的加密格式、FModel 编译与导出、结构解析和预载内容处理经验见 [S4 更新问题与经验总结](./S4_UPDATE_LESSONS.md)。

本文记录 S3 数据更新的实际处理流程，供后续赛季复用。目标是把 FModel 导出的游戏配置和图片安全导入项目，并尽早发现补丁层、本地化、测试形态和资源缺失问题。S4 的结构解码修复、资源变化和验证结果见 [S4_DATA_UPDATE_AUDIT.md](./S4_DATA_UPDATE_AUDIT.md)。
...
- `PETBASE_CONF.form` 中的官方形态描述；该字段非空时必须原样进入 `Pets.json` 和宠物详情，资源文件名只用于字段缺失时兜底推断。
- 非测试或剧情专用的模型/图片引用。
...
- 常规精灵必须能解析到当前 `PET_HANDBOOK` 中真实存在的图鉴条目，并拥有有效战斗数据，才标记为已实装。
...
- 同一中文名的具体形态以 `PETBASE_CONF.form` 为首选标签。同步后应抽查主形态和分支形态，确认没有被统一降级成 `default`、笼统的“异地形态”或“地区形态”。
...
- 没有真实图鉴关联的记录保留为未实装；前端显示“配置 ID”，不得用 PETBASE ID 冒充真实图鉴编号。
...
S4 新资源也使用 `img_…` 键，按原文件名导入；展示工具兼容新键和旧 `JL_…` 键，不重命名资源。
