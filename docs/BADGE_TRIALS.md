...
`/destined-hero-badge` 使用同一份最低阶家族目录，记录某个家族是否已有精灵单人通关命定花种并获得命定勇者徽章。该页面提供已获得/未获得统计、筛选和搜索，进度与草系徽章一起保存及备份。

## 当前功能

- 家族奖牌：按最低阶精灵归并，支持宫格/列表显示、搜索、已点亮/未点亮筛选和每页 24 个家族的翻页显示；手机端宫格每行显示 3 个家族。
- 精灵足迹：当前没有三个地点的完整遭遇名单，因此按地点搜索全量已实装一般形态、地区形态和首领形态，并手动录入为“已点亮”或“未点亮”；已录入结果可分别按点亮状态与形态筛选，每页显示 24 个，手机端宫格每行显示 3 个。“一般形态”筛选包含地区形态。
...
4. 搜索索引仍包含家族全部已实装成员的中文名、内部名和图鉴编号，因此可以用进化后名称定位最低阶家族；纯数字按图鉴编号匹配并支持 `001` 形式，避免与配置 ID 混淆。
...
- 足迹条目只要存在明确 `form` 就显示具体官方形态名，包括同一图鉴编号下最早的主形态；仅当 `form` 为 `default` 时显示“一般形态”。同一图鉴编号存在多个无明确 `form` 的一般形态时，后续条目显示“地区形态 · #配置ID”，用于避免同名形态无法区分。
