...
- 精灵足迹：当前没有三个地点的完整遭遇名单，因此按地点搜索全量已实装一般形态、地区形态和首领形态并手动添加；已录入结果可按全部、一般形态、首领形态筛选。“一般形态”筛选包含地区形态。
...
- 家族键使用最低阶 `species_id`；足迹键使用具体代表配置 ID，格式为 `pet:{id}`，从而区分一般形态、地区形态和首领形态。
