...
- 精灵足迹：当前没有三个地点的完整遭遇名单，因此按地点搜索全量已实装一般形态和首领形态并手动添加；已录入结果可按全部、一般形态、首领形态筛选。
...
- 搜索范围是 `Pets.json` 中全部已实装精灵，包括一般形态和首领形态。
- 用户选择地点后手动添加遇到的精灵，同一精灵可在不同地点分别记录。
- 一般形态按 `species_id` 选择一个稳定代表；首领形态按具体首领配置归类并展示。一般形态与首领形态可以在同一地点分别录入。
- 已录入区域默认显示全部，可切换为只显示一般形态或首领形态；筛选不会改变地点已录入总数。
...
- 家族键使用最低阶 `species_id`；足迹键使用具体代表配置 ID，格式为 `pet:{id}`，从而区分一般形态和首领形态。
